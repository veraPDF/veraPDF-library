...
public class ProcessorImpl implements Processor {
    ...
    private FeaturesCollection extractFeatures(ModelParser parser, Config config) throws FileNotFoundException, JAXBException {
    ...
    private FeaturesConfig getFeaturesConfig(Config config) throws FileNotFoundException, JAXBException {
        ...
        if (!featuresConfigFilePath.toString().isEmpty()) {
            ...
            if (featuresConfigFile.exists() && featuresConfigFile.canRead()) {
                FeaturesConfig featuresConfig = FeaturesConfig.fromXml(new FileInputStream(featuresConfigFile));
                return featuresConfig;
            }
        }

        return new FeaturesConfig.Builder().build();
    }
    ...
    private void writeTextReport(ValidationResult validationResult,
        ...
        if (validationResult != null) {
            ...
            String reportSummary = "ERROR " + fileDetails.getName() +
                    " " + this.validationExeption.toString() + "\n";
            reportOutputStream.write(reportSummary.getBytes());
        }
    }
    ...
    private void writeMRR(ItemDetails fileDetails,
        ...
        if (this.processingResult.getValidationSummary() == ProcessingResult.ValidationSummary.ERROR_IN_VALIDATION) {
            if (this.parsingErrorMessage != null) {
                ...
                machineReadableReport.setErrorInValidationReport("Could not finish validation. " + this.validationExeption.toString());
            } else {
        ...
        if (this.processingResult.getMetadataFixerSummary() == ProcessingResult.MetadataFixingSummary.ERROR_IN_FIXING) {
            if (this.parsingErrorMessage != null) {
                ...
                machineReadableReport.setErrorInMetadataFixerReport("Could not finish metadata fixing. " + this.metadataFixingExeption.toString());
            } else {
        ...
        if (this.processingResult.getFeaturesSummary() == ProcessingResult.FeaturesSummary.ERROR_IN_FEATURES) {
            if (this.parsingErrorMessage != null) {
                ...
                machineReadableReport.setErrorInFeaturesReport("Could not finish features collecting. " + this.featuresExeption.toString());
            } else {
