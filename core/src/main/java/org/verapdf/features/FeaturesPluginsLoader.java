...
public class FeaturesPluginsLoader {
	...
	static void loadExtractors(Path pluginsFolder) {
		File dir = pluginsFolder == null ? null : pluginsFolder.toFile();
	...
	private static List<FeaturesExtractor> loadAllExtractors(List<File> jars) {
		...
		for (File jar : jars) {
			try {
				...
				loadAllExtractorsWithUniqueIDsByClassNames(extractors,
						extractorsIds, jar, classNames);
			} catch (IOException e) {
	...
	private static void loadAllExtractorsWithUniqueIDsByClassNames(
			List<FeaturesExtractor> toAdd, Set<String> uniqueIds, File jar,
